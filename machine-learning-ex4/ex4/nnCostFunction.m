...
a_2 = [ones(1,m); sigmoid(Z_2)];%->26 X 5000
Z_3 = Theta2 * a_2;
...
%J_vec =sum( -Y.*log(h) - ((ones(10,1)-Y).*log(ones(10,1)-h)));
%J = sum(J_vec)/m;
%J = J/m;

% Pay attention the sum is from 2 - without bias thata
T_1 = Theta1(:,2:end).^2;
T_2 = Theta2(:,2:end).^2;

%theta1_sum = sum(T_1(:));
%theta2_sum = sum(T_2(:));
Reg = (lambda / (2*m))*(sum(T_1(:)) + sum(T_2(:)));
J = J+Reg;
%%
%%------------Part 2--------------
...
for t =1:m
    ...
    Theta2_grad = Theta2_grad +  delta_3 * a_2(1:end)';
    Theta1_grad = Theta1_grad +  delta_2 * X(t,1:end);
